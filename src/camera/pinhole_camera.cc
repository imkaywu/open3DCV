...
namespace open3DCV
{
    ...
    PinholeCamera::PinholeCamera(const vector<float>& r_intrinsics, const vector<float>& r_extrinsics) : intrinsics_(r_intrinsics), extrinsics_(r_extrinsics)
    {
        // order does matter
        update_matrices(0);
        update_projection();
    ...
    PinholeCamera::PinholeCamera(const Mat34f& r_projection) : projection_(r_projection)
    {
        // order does matter
    ...
    PinholeCamera::PinholeCamera(const Mat3f& K, const Mat3f& R, const Vec3f& t) : K_(K), R_(R), t_(t)
    {
    ...
    void PinholeCamera::update_projection()
    {
        P_from_KRt(K_, R_, t_, projection_);
    }
    
    void PinholeCamera::update_parameters()
    {
    ...
    void PinholeCamera::update_matrices(const int is_proj)
    {
    ...
    void PinholeCamera::update_center()
    {
        /*
         Mat4f P_homog;
    ...
    // always after update_projection
    void PinholeCamera::update_axes()
    {
    ...
    const Mat34f& PinholeCamera::projection() const
    {
        return projection_;
    }
    
    Mat34f& PinholeCamera::projection()
    {
        return projection_;
    }
    
    const Vec3f& PinholeCamera::direction() const
    {
        return oaxis_;
    }
    
    Vec3f& PinholeCamera::direction()
    {
        return oaxis_;
    }
    
    const Vec3f& PinholeCamera::center() const
    {
        return center_;
    }
    
    Vec3f& PinholeCamera::center()
    {
        return center_;
    }
    
    int PinholeCamera::decompose(Mat3f& K, Mat3f& R) const
    {
        Mat3f M;
    ...
    Vec3f PinholeCamera::pixel2ray(const Vec2f& pixel) const
    {
        return Vec3f(pixel.x(), pixel.y(), -intrinsics_[0]);
    }
    
    Vec3f PinholeCamera::project(const Vec4f& coord) const {
    ...
    // seems not used
    Vec4f PinholeCamera::unproject(const Vec3f& icoord) const {
    ...
    float PinholeCamera::compute_depth(const Vec4f& coord) const {
    ...
    //float PinholeCamera::getScale(const Vec4f& coord, const int level) const {
    //    if (m_maxLevel <= level) {
    ...
    //void PinholeCamera::getPAxes(const Vec4f& coord, const Vec4f& normal, Vec4f& pxaxis, Vec4f& pyaxis, const int level) const {
    ...
    //}
    //
    //void PinholeCamera::setAxisScale(const float axisScale) {
