...
void Camera::init(const string cname, const int maxLevel) {
    m_cname = cname;
    m_maxLevel = maxLevel;
...
Vector3f Camera::project(const Vector4f& coord, const int level) const {
    Vector3f icoord = m_projections[level] * coord;
...
// seems not used
Vector4f Camera::unproject(const Vector3f& icoord, const int level) const {
    Vector3f b(icoord); // the third element should store the depth info
    Matrix3f M = m_projections[level].block(0, 0, 3, 3);
    Vector3f p = m_projections[level].col(3);
    b = b - p;
